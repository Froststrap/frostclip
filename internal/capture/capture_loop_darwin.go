...
func Loop(buf *buffer.CircularBuffer, cfg Config, log *zap.Logger) {
	...
	for {
		...
		go watchSegmentsDarwin(ctx, buf, buf.TempDir(), log)
...
func watchSegmentsDarwin(ctx context.Context, buf *buffer.CircularBuffer, dir string, log *zap.Logger) {
...
func darwinSegReady(path string) bool {
	s1, err := os.Stat(path)
